...
const JobAssetsTab = ({ jobId }) => {
  ...
  // Load assets already related to job
  const {
    data: relatedAssets,
    error: relatedAssetsError,
    isLoading: relatedAssetsLoading,
  } = useGetAssetsForJobQuery(jobId);
  ...
  const handleLinkSelected = async () => {
    try {
      ...
      await linkAssets({ id: jobId, data: assetIds }).unwrap();
  ...
  const handleUnlinkSelected = async () => {
    try {
      const assetIds = selectedAssets.map((asset) => asset.id);
      await unlinkAssets({ id: jobId, data: assetIds }).unwrap();
